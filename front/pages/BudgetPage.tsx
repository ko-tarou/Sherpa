...
const BudgetPage: React.FC<BudgetPageProps> = ({ eventId, event }) => {
  ...
  const totals = useMemo(() => {
    const planned = expenseItems.reduce((s, b) => s + b.planned_amount, 0);
    const actual = expenseItems.reduce((s, b) => s + b.actual_amount, 0);
      ...
      <div className="max-w-6xl mx-auto">
        ...
        <div className="mb-8">
          ...
          <div className="flex flex-wrap gap-8 md:gap-12 mb-4">
            <div>
              <p className="text-xs font-bold text-gray-500 mb-0.5">予算合計</p>
              <p className="text-2xl font-black text-white">{fmt(totals.planned)}</p>
            </div>
            <div>
              <p className="text-xs font-bold text-gray-500 mb-0.5">実績合計</p>
              <p className="text-2xl font-black text-white">{fmt(totals.actual)}</p>
            </div>
            <div>
              <p className="text-xs font-bold text-gray-500 mb-0.5">残高</p>
              <p className={`text-2xl font-black ${totals.remaining >= 0 ? 'text-red-500' : 'text-red-400'}`}>
              ...
              <tbody>
                {[...expenseItems, ...incomeItems].map((b, i) => (
                  <BudgetRow
                    ...
                    onEdit={() => { setMenuId(null); setEditing(b); setShowForm(true); }}
                ...
                {expenseItems.length > 0 && (
                  <tr className="border-t-2 border-white/20 bg-white/5">
                    ...
                    <td className="px-4 py-3 text-white font-bold">{fmt(totals.planned)}</td>
                    <td className="px-4 py-3 text-white font-bold">{fmt(totals.actual)}</td>
...
function BudgetRow({ budget, isEven, menuOpen, onMenuToggle, onEdit, onDelete }: BudgetRowProps) {
  ...
  return (
    <tr className={`border-b border-white/5 ${isEven ? 'bg-white/[0.02]' : 'bg-transparent'}`} data-budget-menu>
      ...
      <td className="px-2 py-2 relative">
        ...
        {menuOpen && (
          <div className="absolute right-2 top-10 z-10 rounded-xl bg-card-bg border border-white/10 shadow-xl py-1 min-w-[120px]">
            <button type="button" onClick={onEdit} className="w-full text-left px-4 py-2 text-sm text-gray-300 hover:bg-white/10 hover:text-white">
              編集
            </button>
            <button type="button" onClick={onDelete} className="w-full text-left px-4 py-2 text-sm text-red-400 hover:bg-white/10">
              削除
            </button>
          </div>
        )}
